...
!!! example "完整的 LR 分析表"
    ...
    根据上面的 DFA 和四条填表规则,可以写出如下完整的 LR 分析表. 表分为两部分:

    - **ACTION 表**: 列是终结符（$x$, $($ , $)$ , $,$ , $\$$）,表项是 shift / reduce / accept
    - **GOTO 表**: 列是非终结符（$S$, $L$）,表项是 goto 到的状态编号

    其中 $s_n$ 表示 shift 到状态 $n$, $r_k$ 表示按第 $k$ 条产生式归约, $g_n$ 表示 goto 状态 $n$, `acc` 表示接受.
    ...
    - **状态 $1$**: 有边 $1 \xrightarrow{x} 2$, $1 \xrightarrow{(} 3$, $1 \xrightarrow{S} 4$
    - **状态 $2$**: 包含 $S \to x.$（产生式 2）,是完成项目,所以对所有终结符填 $r_2$
    - **状态 $3$**: 有边 $3 \xrightarrow{x} 2$, $3 \xrightarrow{(} 3$, $3 \xrightarrow{S} 7$, $3 \xrightarrow{L} 5$
    - **状态 $4$**: 包含 $S' \to S.\$$,遇到 $\$$ 时接受
    - **状态 $5$**: 有边 $5 \xrightarrow{)} 6$, $5 \xrightarrow{,} 8$
    - **状态 $6$**: 包含 $S \to (L).$（产生式 1）,对所有终结符填 $r_1$
    - **状态 $7$**: 包含 $L \to S.$（产生式 3）,对所有终结符填 $r_3$
    - **状态 $8$**: 有边 $8 \xrightarrow{x} 2$, $8 \xrightarrow{(} 3$, $8 \xrightarrow{S} 9$
    - **状态 $9$**: 包含 $L \to L,S.$（产生式 4）,对所有终结符填 $r_4$

    !!! tip "这个文法是 LR(0) 的吗？"

        观察上表,可以发现没有任何一个单元格里出现了两个或以上的动作（也就是没有 shift-reduce conflict 或 reduce-reduce conflict）. 因此这个文法**是 LR(0) 文法**.

        不过要注意,LR(0) 的 reduce 项会填到一整行的所有终结符列中. 如果某个文法在某个状态里既有 shift 又有 reduce,那就会冲突,此时需要用 SLR 或 LR(1) 来解决.
...
!!! note "SLR 的 Reduce 填表规则"

    对于状态 $i$ 中的完成项目 $A \to \alpha .$,假设其产生式编号为 $k$。
    只有当下一个输入符号 $t \in Follow(A)$ 时,才在表项中填写这条 reduce:
    
    $$
    T[i,t] = r_k
    $$
    
    对于不属于 $Follow(A)$ 的终结符列则不填 $r_k$。

比如,如果 $Follow(E)=\{\$\}$,那么就说明在这个文法里,`E` 后面合法出现的符号只有输入结束符 `$`,那么该行就只在 `$` 列填写 `r2`。  
...
=== "Reduce Actions"
    ...
    !!! note "LR(1) 的 Reduce 填表规则"
        ...
        假设该产生式编号为 $k$,那么**只在该项目自带的 lookahead 符号 $z$ 的列**中填写 reduce 动作:
...
!!! info "%union, %token, %type"
    在实验中的.y文件中,我们有如下定义

    === "%union"

        用于声明所有可能的语义值（Semantic Value）类型。
        语法树中不同类型的节点（如整型、字符串、AST 结构体指针等）会具有不同的数据类型，Yacc 会把 `%union` 的内容生成为一个 C 语言的 `union` 联合体，并把全局变量 `yylval` 声明为该联合体类型。
        ```c
        %union {
            int int_val;
            char* str_val;
...
$$
\begin{aligned}
\textit{decl} &\to \textit{type}\ \textit{var-list} \\
\textit{type} &\to \textit{int} \mid \textit{float} \\
\textit{var-list} &\to \textit{var-list},\ \textit{id} \mid \textit{id}
\end{aligned}
$$
...
在实验中,为了保证乘除的优先级大于加法,我们规定一个`Exp`必须先规约成`MulExp`,然后才能规约成`AddExp`.

实际上,Yacc提供了`%left`, `%right`, `%nonassoc`指令来指定优先级和结合性.
...
1. **Token 的优先级**：由声明它的指令（`%left`, `%right`, 等）所处的位置决定，位置越靠下，优先级越高。

2. **Rule 的优先级**：默认由该产生式右侧 **最后一个出现的 Token（终结符）** 决定。
...
!!! tip "%prec"
    ...
    这就要求“单目的减号”不仅要比加减法高，还得比乘法高！但如果直接改 `MINUS` 的优先级，普通的二元减法优先级也会错乱。
    Yacc 给出的一种专门解决这类痛点的方法是，给特定的**单条产生式规则**强行赋予一个优先级——也就是 **`%prec`** 指令。
    ...
    + 例子中出现的 `UMINUS` **永远不会由 Lexer（词法分析器）返回**。它只是我们在 Yacc 中虚构定义出来的一个**占位符 Token**，纯粹是为了在 `%left` 列表中“占个坑”，申请到一个极高的优先级位置。
...
对于局部错误恢复的方法,我们先定义一个`error`的产生式规则:
...
2. **发生错误时退栈**：当解析发生错误时（既无法reduce也无法shift），解析器会不断地弹出符号栈（Stack）顶的操作数，直到暴露出一个能够合法移进（Shift）`error` token 的状态。

3. **移进 `error`**：将 `error` token 压入符号栈中。

4. **不断丢弃非法输入**：解析器为了恢复同步，会开始向后扫描并逐个丢弃（Discard input）输入流中的 token，直到遇到一个可以从当前状态继续进行合法 Shift 的 token 为止（例如遇到了代表同步点的右括号 `)`）。

5. **恢复正常解析**

!!! example "局部错误恢复 - 真题执行轨迹图解"
    假设我们有如下片段的文法规则：
    ```yacc
    exp -> ID
    exp -> exp + exp
    ...
    | `(ID` | `++)$` | reduce `exp -> ID` |
    | `(exp` | `++)$` | shift `+` |
    | `(exp+` | `+)$` | <span style="color:red">**error**</span> |
    | `(exp+` | `error+)$` | <span style="color:red">**pop**</span> |
    | `(exp` | `error+)$` | <span style="color:red">**pop**</span> |
    | `(` | `error+)$` | shift （在这层找到了匹配的 `exp -> ( error )`） |
    | `(error` | `+)$` | <span style="color:red">**discard input**</span> （跳过错误符号 `+`） |
    | `(error` | `)$` | shift |
...
!!! info "Burke-Fisher Error Repair"

    === "核心思想"

        当解析器在某个位置报错卡住时，它不会坐以待毙。它会**往前倒退最多 $K$ 个 token**（这是一个预设的范围窗口）。在这个窗口内的任意位置，它会穷举尝试以下三种**单步操作**：
        ...
        > 这种做法虽然是"穷举法"，只能修复**单一的笔误**（有限 / limited），但在实际编程中**极其有效**，因为大多数语法错误只是漏写了分号、多写了括号等小问题。

    === "评估标准"

        穷举会产生很多种可能的修复方案（比如尝试插入分号、尝试删除逗号等）。

        解析器会在"沙箱"里带着这些修改方案继续往下解析代码。能让解析器在遇到**下一个新错误之前**，往后**走得最远**的那个方案，就是最佳修复方案。
