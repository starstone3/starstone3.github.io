...
这样，一次可以把$\lfloor \frac{b_b}{M} \rfloor -1$个Runs合并为一个新的Runs

- 这样，一共需要的Pass数目为$P = \lceil \log_{\lfloor \frac{b_b}{M} \rfloor-1} (b_r /M) \rceil$

总的Block Transfers为:

$$
C = 2b_r \lceil \log_{\lfloor \frac{b_b}{M} \rfloor-1} {b_r / M} \rceil + b_r
$$
...
- 总的Seek为:
    ...
    - 所以总的寻道次数为:
        ...
        \begin{align}
        2 \lceil \frac{b_r}{M} \rceil + 2\lceil \frac{b_r}{b_b} \rceil \lceil \log_{\lfloor \frac{b_b}{M} \rfloor-1} {b_r / M} \rceil - b_r
        \end{align}
