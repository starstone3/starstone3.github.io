...
在我看来,描述一个光子态的二维振动向量,实际上描述了两件事情:

- X和Y方向振幅大小的相对关系,也就是偏振方向

- X和Y方向振动的相对相位,也就是偏振类型(线偏振/圆偏振/椭圆偏振)

例如,对于振动向量为$(1,3+2i)$的光子:

1. 振幅相对大小为

$$
|E_x|:|E_y|=|1|:|3+2i|=1:\sqrt{13}.
$$

2. 相对相位为

$$
\Delta\phi=\arg(E_y)-\arg(E_x)=\arg(3+2i)-0=\arctan\frac{2}{3}\approx 33.69^\circ.
$$

因此这个态一般是椭圆偏振(既有振幅不等,也有非零相位差)。

若只关心偏振态本身(不关心整体强度),通常会把向量做归一化:

$$
\frac{1}{\sqrt{14}}\begin{pmatrix}1\\3+2i\end{pmatrix}.
$$
...
=E_x\begin{pmatrix}1\\E_y/E_x\end{pmatrix}.
$$

这说明同一个偏振态总能写成$(1,z)$的形式($z=E_y/E_x$),也就是“X分量可以变成1”。  
本质上这是把整体复系数(对应整体振幅和公共相位)提出来,不改变偏振的相对信息。

唯一例外是$E_x=0$,此时态是纯$y$偏振,可写成$(0,1)$,不能再除以$E_x$。
