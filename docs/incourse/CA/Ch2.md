...
1. 在`Issue`阶段,Scoreboard检查指令需要使用的功能单元是否空闲,如果空闲,且不存在`WAW`冒险,就把指令发射,同时,更新上面这三张表:
...
<div align="center">
    <img src="../../../image/mac21.png" alt="ROB" width="2000"/>
    <br>
    <caption>Reorder Buffer</caption>
</div>

可以发现,与原始的Tomasulo相比,主要有以下变化:

1. CDB总线现在通往`ROB`,而不是寄存器堆.

2. 保留站现在可以从`ROB`中读取数据.

3. ROB实际上就是一个FIFO队列,它的每一行都可以存放一条指令的结果,先进先出.
...
1. `Insrtuction Type`:指令类型,比如`load`或`store`还是`branch`

2. `Destionation`:目的寄存器,如果是`store`指令,则是存储地址.

3. `Value`:指令的结果,比如`ALU`操作的结果,`branch`指令的目标地址等.

4. `Ready`:指令是否完成.


包含了`ROB`的Tomasulo算法的由如下几个阶段组成:
...
1. `Issue`:
    ...
    - 同时,`ROB.push_back(inst)`,将指令放入`ROB`中.

    - 需要注意的是,此时寄存器状态表中的编号不再是保留站的编号,而是指令在`ROB`中的编号.

    - 同时,保留站中的`Qj`和`Qk`也不再是其他保留站的编号,而是`ROB`中的编号.
...
3. `Write Back`:
    ...
    - 指令执行完之后直接写到`ROB`中,同时数据在CDB上广播.

4. `Commit`:

    - 当队列头部的指令`Ready`时,就可以提交了.

    - 把指令的结果写到寄存器中

    - 如果是`branch`指令,这检查是否预测正确,如果不正确,则清空`ROB`中的所有指令.
    
    - 如果是`store`指令,则将数据写入内存中

    - 当然,如果头部的指令一直没有`Ready`,就一直等待.
